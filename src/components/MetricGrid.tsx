    ...
    <div className="grid gap-2">
      {primaryItem ? (
        <div className="grid gap-1 border-b border-(--line) pb-4">
          ...
          <strong className="block font-serif text-4xl leading-none tracking-tight text-(--teal)">
            {primaryItem.value}
          </strong>
        </div>
      ) : null}
      ...
      {items.map((item: MetricGridItem, index: number) => (
        <div key={item.label} className={index > 0 ? "border-t border-(--line) pt-2" : primaryItem ? "pt-2" : ""}>
