...
function ProjectionLineChart({ ariaLabel, currentYear, inputs, projection, series, valueDomain = "span" }) {
  ...
  return (
    <svg viewBox="0 0 720 320" role="img" aria-label={ariaLabel}>
      ...
      {series.map((item) => (
        <path
          key={item.key}
          d={buildLinePath(displayProjection.map((row, index) => pointFor(row.values[item.key], index)))}
          fill="none"
          stroke={item.stroke}
...
export function NetWorthChart({ inputs, results, currentYear }) {
  return (
    <ProjectionLineChart
...
export function AssetTaxChart({ inputs, results, currentYear }) {
  return (
    <ProjectionLineChart
