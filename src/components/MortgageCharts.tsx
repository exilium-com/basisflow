...
import { type MortgageScenario, type MortgageYearlyBreakdownRow } from "../lib/mortgageSchedule";
...
function EmptyChart({ message, label }: EmptyChartProps) {
  return (
...
export function MortgageBalanceChart({ scenario }: MortgageChartProps) {
...
export function MortgageCompositionChart({ scenario }: MortgageChartProps) {
  ...
  const { height, innerWidth, innerHeight, plotLeft, plotTop, plotRight, plotBottom } = getChartFrame();
  const maxTotal = Math.max(...breakdown.map((year: MortgageYearlyBreakdownRow) => year.principal + year.interest), 1);
  ...
  return (
    <svg viewBox="0 0 720 320" role="img" aria-label="Principal versus interest by year chart">
      ...
      {breakdown.map((year: MortgageYearlyBreakdownRow, index: number) => {
