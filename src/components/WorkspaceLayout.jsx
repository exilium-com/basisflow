...
export function WorkspaceLayout({ summary, children }) {
  ...
  return (
    <section className="grid min-h-0 flex-1 items-start lg:grid-cols-5">
      <div ref={contentRef} className="min-h-0 self-stretch p-6 lg:col-span-3 max-lg:p-4">
        <div className="pb-6 max-lg:pb-5">{children}</div>
      </div>
      <aside
        className="sticky top-0 self-start overflow-y-auto border-l border-(--line) bg-(--white-soft) p-6 lg:col-span-2 max-lg:static max-lg:h-auto max-lg:self-stretch max-lg:overflow-visible max-lg:border-l-0 max-lg:border-t max-lg:border-t-(--line) max-lg:p-4"
